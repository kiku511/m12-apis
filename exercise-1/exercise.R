...
response <- GET("http://data.unhcr.org/api/stats/country_of_residence.json")
countries <- fromJSON(content(response, "text"))
filter(countries, country_of_residence_en == "Syrian Arab Republic") %>%
...
plot(refugees.usa, type = "o", col = "blue")
...
turkey.code <- countries %>% filter(country_of_residence_en == "Turkey") %>% select(country_of_residence) %>% .[[1,1]]
query.params <- list(country_of_residence = turkey.code, country_of_origin = "SYR", population_type_code = "RF")
response <- GET("http://data.unhcr.org/api/stats/time_series_all_years.json?", query = query.params)
refugees.tur <- fromJSON(content(response, "text"))
refugees.tur <- select(refugees.tur, year, tur = value)
lines(refugees.tur, type = "o", col = "red")
